...
export function EditBrandPage({ brandId }: EditBrandPageProps) {
  ...
  const formValues = brandData
    ? {
      ...
      // Добавляем категории
      categoryIds: brandData.categories?.map(cat => cat.id) || [],
      ...
      logoMeta: brandData.files?.find((file) => file.type === "logo")
        ? {
          name:
            brandData.files?.find((file) => file.type === "logo")?.file?.name ??
            "",
          mimeType:
            brandData.files?.find((file) => file.type === "logo")
              ?.file?.mimeType ?? "",
          size:
            brandData.files?.find((file) => file.type === "logo")?.file?.size ?? 0,
          url:
      ...
      bannerMeta: brandData.files?.find((file) => file.type === "banner")
        ? {
          name:
            brandData.files?.find((file) => file.type === "banner")?.file?.name ??
            "",
          mimeType:
            brandData.files.find((file) => file.type === "banner")
              ?.file?.mimeType ?? "",
          size:
            brandData.files.find((file) => file.type === "banner")?.file?.size ??
            0,
          url:
